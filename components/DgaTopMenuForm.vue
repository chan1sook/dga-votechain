<template>
  <div class="grid grid-cols-12">
    ...
    <div class="col-span-1 flex flex-col gap-2 justify-center items-center">
      ...
      <button class="disabled:text-gray-400" :title="$t('app.deselect')" :disabled="!selectedCurrentOption" @click="deselectCurrentTopMenu">
        <ChevronLeftIcon></ChevronLeftIcon>
      </button>
      <button class="disabled:text-gray-400" :title="$t('app.select')" :disabled="!selectedRemainOption" @click="selectCurrentTopMenu">
    ...
    <div class="col-span-1 flex flex-col gap-2 justify-center items-center">
      <button :title="$t('app.moveToTop')" class="disabled:text-gray-400"
        :disabled="!selectedCurrentOption" @click="moveCurrentTopMenuToTop">
        <ChevronDoubleUpIcon></ChevronDoubleUpIcon>
      </button>
      <button :title="$t('app.moveUp')" class="disabled:text-gray-400" 
        :disabled="!selectedCurrentOption" @click="moveCurrentTopMenuUp">
        <ChevronUpIcon></ChevronUpIcon>
      </button>
      <button :title="$t('app.moveDown')" class="disabled:text-gray-400"
        :disabled="!selectedCurrentOption" @click="moveCurrentTopMenuDown">
        <ChevronDownIcon></ChevronDownIcon>
      </button>
      <button :title="$t('app.moveToBottom')" class="disabled:text-gray-400"
...
const remainTopMenuOptions = computed(() => {
  return props.menuOptions.filter((ele) => currentTopMenus.value.indexOf(ele) === -1)
});
