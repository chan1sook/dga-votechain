  ...
  <nav class="dga-nav">
    ...
    <div class="flex flex-row gap-2 overflow-hidden">
      <div v-for="menu of currentTopMenus">
        <NuxtLink v-if="menu === 'home'" :href="localePathOf('/')" class="dga-menu-item">
          {{ $t("app.home.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'voting'" :href="localePathOf('/topics')" class="dga-menu-item">
          {{ $t("app.voting.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'about'" :href="localePathOf('/about')" class="dga-menu-item">
          {{ $t("app.about.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'help'" :href="localePathOf('/help')" class="dga-menu-item">
          {{ $t("app.help.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'contact-us'" :href="localePathOf('/contact-us')" class="dga-menu-item">
          {{ $t("app.contactUs.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'users-management'" :href="localePathOf('/admin/users')" class="dga-menu-item">
          {{ $t("app.navbar.adminShowUsers") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'blockchain'" :href="localePathOf('/admin/blockchain')" class="dga-menu-item">
          {{ $t("app.navbar.blockchain") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'server-config'" :href="localePathOf('/admin/config')" class="dga-menu-item">
          {{ $t("app.admin.config.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'monitor'" :href="localePathOf('/admin/monitor')" class="dga-menu-item">
          {{ $t("app.admin.monitor.title") }}
        </NuxtLink>
      </div>
    </div>
    <div class="ml-auto inline-flex flex-row items-center gap-4">
      <div class="hidden flex-row gap-2 xl:flex">
        <DgaButton v-if="isAdmin || isDeveloper" color="dga-orange" :theme="roleMode !== 'voter' ? 'hollow' : undefined"
          class="whitespace-nowrap !py-1 !text-sm" :title="switchRoleStrOf('voter')" @click="switchRoleMode('voter')">
          {{ $t("app.role.voter") }}
        </DgaButton>
        <DgaButton v-if="isAdmin" color="dga-orange" :theme="roleMode !== 'admin' ? 'hollow' : undefined"
          class="whitespace-nowrap !py-1 !text-sm" :title="switchRoleStrOf('admin')" @click="switchRoleMode('admin')">
          {{ $t("app.role.admin") }}
        </DgaButton>
        <DgaButton v-if="isDeveloper" color="dga-orange" :theme="roleMode !== 'developer' ? 'hollow' : undefined"
          class="whitespace-nowrap !py-1 !text-sm" :title="switchRoleStrOf('developer')"
    ...
    <div v-if="showMenuOption" class="collasped-menu px-4 py-2 lg:hidden">
      <template v-for="menu of currentTopMenus">
        <NuxtLink v-if="menu === 'home'" :href="localePathOf('/')" class="dga-menu-item-small">
          {{ $t("app.home.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'voting'" :href="localePathOf('/topics')" class="dga-menu-item-small">
          {{ $t("app.voting.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'about'" :href="localePathOf('/about')" class="dga-menu-item-small">
          {{ $t("app.about.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'help'" :href="localePathOf('/help')" class="dga-menu-item-small">
          {{ $t("app.help.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'contact-us'" :href="localePathOf('/contact-us')" class="dga-menu-item-small">
          {{ $t("app.contactUs.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'users-management'" :href="localePathOf('/admin/users')"
          class="dga-menu-item-small">
          {{ $t("app.navbar.adminShowUsers") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'blockchain'" :href="localePathOf('/admin/blockchain')" class="dga-menu-item-small">
          {{ $t("app.navbar.blockchain") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'server-config'" :href="localePathOf('/admin/config')" class="dga-menu-item-small">
          {{ $t("app.admin.config.title") }}
        </NuxtLink>
        <NuxtLink v-else-if="menu === 'monitor'" :href="localePathOf('/admin/monitor')" class="dga-menu-item-small">
          {{ $t("app.admin.monitor.title") }}
        </NuxtLink>
      </template>
    </div>
...
<style scoped>
.dga-nav {
  @apply sticky top-0 z-[100] flex h-16 select-none flex-row items-center gap-4 border border-gray-500/25 bg-white p-4 shadow transition duration-200 hover:shadow-md lg:h-20;
}
