<template>
  <div class="relative" @click.stop>
    <button type="button" @click="toggleShowOption" :title="$t('app.navbar.news.title')">
      <NewspaperIcon />
    </button>
    <div v-if="showOption" class="z-[400] bg-white border rounded-md rounded-b-3xl shadow fixed right-0 top-16 lg:top-20 w-72 max-h-[400px] overflow-y-auto" @click.stop>
      <div class="flex-1 flex flex-col gap-2 px-4 py-2">
        ...
        <div v-if="isDeveloperRole(roleMode)" class="absolute right-0 top-2">
          <NuxtLink :href="localePathOf('/news/create')" :title="$t('app.navbar.news.add')" @click="useVisibleMenuGroup().value = undefined">
            <PlusIcon />
          </NuxtLink>
        </div>
        ...
        <template v-else>
          ...
          <DgaButton v-if="hasMoreNews && isLoadMoreNews" :title="$t('app.navbar.news.loadMoreNews')" color="dga-orange" class="mx-auto" @click="loadMoreNews">
