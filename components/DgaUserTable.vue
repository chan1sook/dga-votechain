<template>
  <div class="flex flex-col gap-2">
    <div class="overflow-auto max-h-[50vh]">
      <div class="user-grid" :class="[props.multipleVotes && !props.coadmin ? 'multichoice' : '']">
        <div class="font-bold"></div>
        <div class="font-bold">{{ $t('app.userid') }}</div>
        <div class="font-bold">{{ $t('app.userName') }}</div>
        <div class="font-bold">{{ $t('app.email') }}</div>
        <div v-if="props.multipleVotes && !props.coadmin" class="font-bold">{{ $t('app.voterList.totalVotes')}}</div>
        <div></div>
        ...
        <template v-for="user of props.users">
          ...
          <div>
            <button class="align-middle px-2 py-1 inline-flex items-center justify-center"
              :title="`${$t('app.voterList.remove')} [${getFullName(user)}]`"  @click="emit('remove', user)"
            >
              <MinusIcon/>
            </button>
          </div>
