...
export default defineEventHandler(async (event) => {
  ...
  const { permissions }: { permissions: EVotePermission[] } = await readBody(
    event
  );
  ...
  const permissionDiffA = removePermissions(
    userDoc.permissions,
    ...permissions
  );
  const permissionDiffB = removePermissions(
    permissions,
    ...userDoc.permissions
  );
  const permissionDiff = combinePermissions(
    permissionDiffA,
    ...permissionDiffB
  );

  if (
    userDoc._id.toString() === userData._id.toString() &&
    checkPermissionSelections(
      permissionDiff,
