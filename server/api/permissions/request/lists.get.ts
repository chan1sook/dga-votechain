...
export default defineEventHandler(async (event) => {
  ...
  const reqPermissionData: RequestPermissionsModelDataWithPopulated[] =
    await getPendingRequestPermissionsData(pagesize, startid).populate(
      "userid"
    );

  const requestPermissions: RequestPermissionsListData[] =
    reqPermissionData.map((doc) => {
      return {
        _id: `${doc._id}`,
        status: doc.status,
