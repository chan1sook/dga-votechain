...
export default defineEventHandler(async (event) => {
  ...
  if (
    ...
    !checkPermissionSelections(userData.permissions, "change-topic")
  ) {
    throw createError({
      statusCode: 403,
  ...
  if (
    admins.findIndex((ele) => ele.toString() === userData._id.toString()) === -1
  ) {
    throw createError({
      statusCode: 403,
      statusMessage: "Forbidden",
  ...
  const voterAllowDocs = await TopicVoterAllowsModel.find({
    topicid: topicDoc._id,
  });
