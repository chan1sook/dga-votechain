...
export default defineEventHandler(async (event) => {
  ...
  if (userData && !isBannedUser(userData)) {
    const _yourVotes = votes.filter(
      (ele) => ele.userid && ele.userid.toString() === userData._id.toString()
    );
  ...
  const listedUserVotes = votes.filter((vote) => {
    return (
      vote.userid &&
      votersData.find(
        (voter) => voter.userid._id.toString() === vote.userid?.toString()
      )
    );
  });

  const anonVotes = votes
    .filter((vote) => !listedUserVotes.includes(vote))
    .map((ele) => {
  ...
  const voterVoted =
    voterTotal -
    votersData.filter((voter) => {
      return (
        voter.remainVotes >= voter.totalVotes &&
        !votes.find((vote) => {
          return (
            vote.userid &&
            vote.userid.toString() === voter.userid._id.toString()
          );
        })
      );
    }).length;
  ...
  const voteResult: TopicResultResponse = {
    ...
    stats: {
      ...
      votes: {
        quota: votersData.reduce(
          (prev, current) => current.totalVotes + prev,
          0
        ),
