...
export default defineEventHandler(async (event) => {
  ...
  const userSearchKeywords = await new Promise<CSVSearchParams[]>(
    (resolve, reject) => {
      const searchParams: CSVSearchParams[] = [];

      Readable.from(tempFile)
  ...
  const searchUsers = await batchSearchActiveUserByKeywords(
    userSearchKeywords,
    { adminOnly, excludeUserId }
  );

  const users = searchUsers.map((user) => {
    let role: UserRole | undefined;
    if (isUserDeveloper(userData)) {
      role = isUserDeveloper(user.user)
        ? "developer"
